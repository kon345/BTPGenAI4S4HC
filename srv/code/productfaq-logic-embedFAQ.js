...
/**
 * 
 * @After(event = { "CREATE","UPDATE" }, entity = "henryLin0610_STUDENTG20Srv.ProductFAQ")
...
try {
    ...
    productFAQ = await SELECT.one('HenryLin0610_STUDENTG20.ProductFAQ').where({ ID: productFAQID }).forUpdate();
    if (!productFAQ) {
...
try {
    ...
    await UPDATE('HenryLin0610_STUDENTG20.ProductFAQ').set({ embedding: embedding }).where({ ID });
