...
const { generateResponseTechMessage,
...
/**
 * 
 * @On(event = { "Action1" }, entity = "henryLin0610_STUDENTG20Srv.CustomerMessage")
...
try {
    ...
    customerMessage = await SELECT.one.from('HenryLin0610_STUDENTG20.CustomerMessage').where({ ID });
    if (!customerMessage) {
...
if (messageCategory === 'Technical') {
    ...
    try {
        ...
        relevantFAQs = await SELECT.from('HenryLin0610_STUDENTG20.ProductFAQ')
...
try {
    ...
    await UPDATE('HenryLin0610_STUDENTG20.CustomerMessage').set({
